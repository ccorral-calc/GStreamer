...
public:
        ...
        : port_(port), filename_(filename), keep_running_(true), data_flowing_(false)
    {
        ...
        // Define elements
        std::string input_selector = "input-selector name=sel sync-streams=true sync-mode=clock cache-buffers=true ! h264parse config-interval=1 ! mpegtsmux ! appsink name=mysink emit-signals=true max-buffers=1 drop=true ";
    ...
    // Callback to handle new data buffers
...
Config parse_args(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        {
            printf("-n Number of Streams\r\n-p Starting Port up to N streams\r\n-h help\r\n");
            exit(1);
        }
    }
    return cfg;
